...
def linear_train(train_features, train_labels,lr=0.0075, num_classes = 10, num_epochs = 100):
...
def linear_train_combined(train_features1, train_features2, train_labels,lr=0.0075, num_classes = 10, num_epochs = 100):
    
    train_data = tensor_dataset_combined(train_features1,train_features2,train_labels)
   
    train_loader = DataLoader(train_data, batch_size=100, shuffle=True, drop_last=True, num_workers=2)


    LL = nn.Linear(train_features1.shape[1],num_classes)
    optimizer = torch.optim.SGD(LL.parameters(), lr=lr, momentum=0.9, weight_decay=5e-5)
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, 100)
    ...
    for epoch in range(num_epochs):
        ...
        for counter, (x_batch, y_batch) in enumerate(train_loader):
            ...
            logits = 0.5*LL(x_batch1)+0.5*LL(x_batch2)
            loss = criterion(logits, y_batch)
            top1 = accuracy(logits, y_batch, topk=(1,))
            top1_train_accuracy += top1[0]
            optimizer.zero_grad()
...
class tensor_dataset_combined(data.Dataset):
